...
const AgentPaymentHistory = ({ 
    formatCurrency, sumOfPayments, 
    chosenAgent, formatDate, allPayments, 
  ...
  const setAllComponents = () =>
  {
    const  col = [
        ...
        },        
        {
          ...
          width: 100,
          multiple: 1,
          render: (text, record, index) => <PaymentButton handleDeletePayment={handleDeletePayment} setSelectedItemForDelete={setSelectedItemForDelete} text={`${text}`} />,
        }
      ];
      setColumns(col);
  }
      ...
      <div className="flex flex-col shadow-md p-2 bg-[conic-gradient(at_top,_var(--tw-gradient-stops))] from-red-600 via-red-50 to-red-600">
        <div className='flex items-center justify-between'>
          <h1 className="text-lg ml-2 shadow-2 text-white uppercase tracking-widest">
              List of Payments for Agent { chosenAgent && (`${chosenAgent.first_name} ${chosenAgent.last_name}`) }
          </h1>
          <h1 className='text-lg shadow-2 border border-white shadow-sm text-white uppercase px-2 py-1 bg-red-500 rounded-md'>Total Payment: { formatCurrency(sumOfPayments, 'PHP') } | Balance: { computeBalance(chosenAgent.price, sumOfPayments, 'PHP') }</h1>
        </div>
            <div className="pt-2">
              <Table                
                rowKey='id'                 
